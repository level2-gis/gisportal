--gisapp upgrade script v22

INSERT INTO settings (version, date) VALUES (22, now());

ALTER TABLE project_groups ADD COLUMN custom3 text;
ALTER TABLE project_groups ADD COLUMN custom4 text;
ALTER TABLE project_groups ADD COLUMN link1 text;
ALTER TABLE project_groups ADD COLUMN link2 text;
ALTER TABLE project_groups ADD COLUMN link3 text;

DROP FUNCTION IF EXISTS public.get_project_data(text);
CREATE OR REPLACE FUNCTION public.get_project_data(project text)
  RETURNS TABLE(client_id integer, client_name text, client_display_name text, client_url text, theme_name text, overview_layer json, base_layers json, extra_layers json, tables_onstart text[], is_public boolean, project_id integer, project_name text, project_display_name text, crs text, description text, restrict_to_start_extent boolean, geolocation boolean, feedback boolean, measurements boolean, print boolean, zoom_back_forward boolean, identify_mode boolean, permalink boolean, feedback_email text, project_path text, plugins text[], custom1 text, custom2 text, custom3 text, custom4 text, link1 text, link2 text, link3 text)
LANGUAGE 'plpgsql'
